...
const Talents = () => {
  ...
  useEffect(() => {
    ...
    getAllProfiles().then((res) => {
      console.log("Fetched profiles:", res); // Log fetched profiles
      setTalents(res);
    }).catch((err) => {
  ...
  useEffect(() => {
    ...
    // console.log("Filtered talents:", filterTalent); // Log filtered talents
    ...
    <div className='p-5'>
      ...
      <div className='mt-10 flex flex-wrap gap-5 justify-center'>
        {filteredTalents.length > 0 ? (
          ...
          <div>No talents found</div> // Handle case when no talents are found
        )}
      </div>
    </div>
  );
